...
```swift
func traversal(node: TreeNode?, inorderValues: inout [Int]) {
      guard let node = node else { return }
      traversal(node: node.left, inorderValues: &inorderValues) // 左
      inorderValues.append(node.val) // 中
      traversal(node: node.right, inorderValues: &inorderValues) // 右
  }
  func isValidBST(_ root: TreeNode?) -> Bool {
      var inorderValues = [Int]()
      traversal(node: root, inorderValues: &inorderValues)
      for i in 1 ..< inorderValues.count {
          guard inorderValues[i] > inorderValues[i - 1] else { return false }
      }
      return true
  }
```
...
```swift
func traversal(node: TreeNode?, previousNode: inout TreeNode?) -> Bool {
    guard let node = node else { return true }
    // 左
    let left = traversal(node: node.left, previousNode: &previousNode)
    // 中
    if let previousValue = previousNode?.val, previousValue >= node.val { return false }
    previousNode = node
    // 右
    let right = traversal(node: node.right, previousNode: &previousNode)
    // 汇总
    return left && right
}
func isValidBST(_ root: TreeNode?) -> Bool {
    var previousNode: TreeNode?
    return traversal(node: root, previousNode: &previousNode)
}
...
func isValidBST(_ root: TreeNode?) -> Bool {
    ...
    while current != nil || !stack.isEmpty {
        if let node = current {
            ...
            current = stack.removeLast() // 中
            if let previousValue = previous?.val, previousValue >= current!.val {
                return false
            }
            previous = current
