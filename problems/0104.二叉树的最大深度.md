...
```swift
func getDepth(node: TreeNode?, depth: Int, maxDepth: inout Int) {
    maxDepth = max(maxDepth, depth) // 中：回收结果

    if let node = node?.left { // 左，需要回溯
        getDepth(node: node, depth: depth + 1, maxDepth: &maxDepth)
    }
...
func maxDepth(_ root: TreeNode?) -> Int {
    ...
    getDepth(node: node, depth: 1, maxDepth: &result)
    return result
}
```
