...
```swift
func buildTree0(_ inorder: [Int], _ postorder: [Int]) -> TreeNode? {
    // 1. 数组为空则为空结点
    guard !postorder.isEmpty else { return nil }

    // 2. 取出后序数组末尾作为根结点
    var inorder = inorder, postorder = postorder
    let root = TreeNode(postorder.removeLast())

    // 叶子结点，直接返回
    guard !postorder.isEmpty else { return root }

    // 3. 用根结点切割中序数组
    let rootIndex = inorder.firstIndex(of: root.val)!
    let subinorders = [inorder[0 ..< rootIndex], inorder[rootIndex + 1 ..< inorder.count]].map { Array($0) }
    // 4. 利用后序数组大小与中序数组大小相等的特性，用数组大小切割后序数组
    let subpostorders = [postorder[0 ..< subinorders[0].count], postorder[subinorders[0].count ..< postorder.count]].map { Array($0) }

    // 5. 递归
    root.left = buildTree(subinorders[0], subpostorders[0])
    root.right = buildTree(subinorders[1], subpostorders[1])
...
```swift
typealias IndexRange = (start: Int, end: Int)
func traversal(inorder: [Int], inorderRange: IndexRange, postorder: [Int], postorderRange: IndexRange) -> TreeNode? {
    guard postorderRange.end >= postorderRange.start else { return nil }

    let root = TreeNode(postorder[postorderRange.end])
    let rootIndex = inorder.firstIndex(of: root.val)!
    let leftInoderRange = (inorderRange.start, rootIndex - 1)
    let rightInoderRnage = (rootIndex + 1, inorderRange.end)
    let leftPostoderRange = (postorderRange.start, postorderRange.start + leftInoderRange.1 - leftInoderRange.0)
    let rightPostoderRange = (leftPostoderRange.1 + 1, postorderRange.end - 1)

    root.left = traversal(inorder: inorder, inorderRange: leftInoderRange, postorder: postorder, postorderRange: leftPostoderRange)
    root.right = traversal(inorder: inorder, inorderRange: rightInoderRnage, postorder: postorder, postorderRange: rightPostoderRange)

    return root
}
func buildTree(_ inorder: [Int], _ postorder: [Int]) -> TreeNode? {
    traversal(inorder: inorder, inorderRange: (0, inorder.count - 1), postorder: postorder, postorderRange: (0, postorder.count - 1))
}
```
...
func buildTree(_ preorder: [Int], _ inorder: [Int]) -> TreeNode? {
    ...
    // 2. 取出前序数组开头作为根结点
    var inorder = inorder, preorder = preorder
    let root = TreeNode(preorder.removeFirst())

    // 叶子结点，直接返回
    guard !preorder.isEmpty else { return root }

    // 3. 用根结点切割中序数组
    let rootIndex = inorder.firstIndex(of: root.val)!
    let subinorders = [inorder[0 ..< rootIndex], inorder[rootIndex + 1 ..< inorder.count]].map { Array($0) }
    // 4. 利用后序数组大小与中序数组大小相等的特性，用数组大小切割后序数组
    let subpreorders = [preorder[0 ..< subinorders[0].count], preorder[subinorders[0].count ..< preorder.count]].map { Array($0) }

    // 5. 递归
    root.left = buildTree(subpreorders[0], subinorders[0])
    root.right = buildTree(subpreorders[1], subinorders[1])
...
```swift
func traversal(preorder: [Int], preorderIndex: IndexRange, inorder: [Int], inorderIndex: IndexRange) -> TreeNode? {
    guard preorderIndex.end >= preorderIndex.start else { return nil }

    let root = TreeNode(preorder[preorderIndex.start])
    let rootIndex = inorder.firstIndex(of: root.val)!
    let leftInorderIndex = (inorderIndex.start, rootIndex - 1)
    let rightInorderIndex = (rootIndex + 1, inorderIndex.end)
    let leftPreorderIndex = (preorderIndex.start + 1, preorderIndex.start + 1 + leftInorderIndex.1 - leftInorderIndex.0)
    let rightPreorderIndex = (leftPreorderIndex.1 + 1, preorderIndex.end)

    root.left = traversal(preorder: preorder, preorderIndex: leftPreorderIndex, inorder: inorder, inorderIndex: leftInorderIndex)
    root.right = traversal(preorder: preorder, preorderIndex: rightPreorderIndex, inorder: inorder, inorderIndex: rightInorderIndex)

    return root
}
func buildTree(_ preorder: [Int], _ inorder: [Int]) -> TreeNode? {
    traversal(preorder: preorder, preorderIndex: (0, preorder.count - 1), inorder: inorder, inorderIndex: (0, inorder.count - 1))
}
```
