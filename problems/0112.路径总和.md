...
```swift
func traversal(node: TreeNode, remain: Int) -> Bool {
    if node.left == nil, node.right == nil { // 中：叶子结点结束
        return remain == 0
    }

    var remain = remain
    if let node = node.left { // 左，需要回溯
        remain -= node.val
        if traversal(node: node, remain: remain) { return true }
        remain += node.val
    }
    if let node = node.right { // 右，需要回溯
        remain -= node.val
        if traversal(node: node, remain: remain) { return true }
        remain += node.val
    }
...
func hasPathSum(_ root: TreeNode?, _ targetSum: Int) -> Bool {
    ...
    // 已经在传参时候减去了根结点值
    return traversal(node: node, remain: targetSum - node.val)
}
```
...
func hasPathSum(_ root: TreeNode?, _ targetSum: Int) -> Bool {
    ...
    // 中：必须要先-1，把-1放到参数会遗漏只有一个结点的树
    let targetSum = targetSum - node.val
    if node.left == nil, node.right == nil { // 叶子结点
        return targetSum == 0
    }

    if let node = node.left { // 左，需要回溯
        if hasPathSum(node, targetSum) { return true }
    }
    if let node = node.right { // 右，需要回溯
        if hasPathSum(node, targetSum) { return true }
    }
...
func hasPathSum(_ root: TreeNode?, _ targetSum: Int) -> Bool {
    guard let node = root else { return false }
    var stack = [(node, node.val)]
    while !stack.isEmpty {
        let (node, sum) = stack.removeLast()
        if let node = node.right {
            stack.append((node, sum + node.val)) // 右
        }
        if let node = node.left {
            stack.append((node, sum + node.val)) // 左
        }
        // 中：叶子结点，这里sum == targetSum挪到下面返回就不行！
...
```swift
func traversal(node: TreeNode, remain: Int, path: inout [Int], result: inout [[Int]]) {
    // 遇到叶子结点结束
    if node.left == nil, node.right == nil {
        // 中：回收结果
    ...
    var remain = remain
    if let node = node.left { // 左
        path.append(node.val)
        remain -= node.val
        traversal(node: node, remain: remain, path: &path, result: &result)
        remain += node.val // 回溯
        path.removeLast() // 回溯
    }
    if let node = node.right { // 右
        path.append(node.val)
        remain -= node.val
        traversal(node: node, remain: remain, path: &path, result: &result)
        remain += node.val // 回溯
...
func pathSum(_ root: TreeNode?, _ targetSum: Int) -> [[Int]] {
    ...
    var path = [node.val] // 把根节点放进路径
    var result = [[Int]]()
    traversal(node: node, remain: targetSum - node.val, path: &path, result: &result)
    return result
}
```
...
func traversal(node: TreeNode?, remain: Int, path: inout [Int], result: inout [[Int]]) {
    ...
    // 递归左、右
    traversal(node: node.left, remain: remain, path: &path, result: &result)
    traversal(node: node.right, remain: remain, path: &path, result: &result)
    path.removeLast() // 回溯
}
func pathSum(_ root: TreeNode?, _ targetSum: Int) -> [[Int]] {
    var path = [Int]()
    var result = [[Int]]()
    traversal(node: root, remain: targetSum, path: &path, result: &result)
    return result
}
```
