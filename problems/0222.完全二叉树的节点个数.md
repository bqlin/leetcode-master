...
那么只要模板稍做改动，加一个变量result，统计节点数量就可以了
...
func countNodes(_ root: TreeNode?) -> Int {
    ...
    let leftCount = countNodes(node.left) // 左
    let rightCound = countNodes(node.right) // 右
    return leftCount + rightCound + 1 // 中
}
```
...
func countNodes(_ root: TreeNode?) -> Int {
    ...
    var tmp = root.left
    while tmp != nil { // 计算左子树深度
        tmp = tmp?.left
        leftDepth += 1
    }
    tmp = root.right
    while tmp != nil { // 计算右子树深度
        tmp = tmp?.right
        rightDepth += 1
    }
    if leftDepth == rightDepth { // 满二叉树（单个叶子结点也是满二叉树）
