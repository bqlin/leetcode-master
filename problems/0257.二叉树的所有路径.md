...
```swift
func traversal(node: TreeNode?, path: inout [Int], result: inout [String]) {
    guard let node = node else { return } // 中
    ...
    if let node = node.left { // 左
        traversal(node: node, path: &path, result: &result)
        path.removeLast() // 回溯
    }
...
func binaryTreePaths(_ root: TreeNode?) -> [String] {
    var result = [String]()
    var path = [Int]()
    traversal(node: root, path: &path, result: &result)
    return result
}
```
...
```swift
func traversal1(node: TreeNode?, path: inout String, result: inout [String]) {
    ...
    // 注意这里的path是创建了一个新变量，不能再原有的变量上做修改，否则就没有回溯
    if let node = node.left {
        var path = path + "->"
        traversal1(node: node, path: &path, result: &result)
    }
...
func binaryTreePaths(_ root: TreeNode?) -> [String] {
    var result = [String]()
    var path = ""
    traversal1(node: root, path: &path, result: &result)
    return result
}
```

前序迭代，这个版本感觉逻辑更清晰，但把回溯的细节隐藏起来了。

```swift
func binaryTreePaths(_ root: TreeNode?) -> [String] {
    var result = [String]()
    guard let root = root else { return result }
    var stack = [(root, String(root.val))]
    while !stack.isEmpty {
        let (node, path) = stack.removeLast()
        if let node = node.right { // 右
            stack.append((node, "\(path)->\(node.val)"))
        }
        if let node = node.left { // 左
            stack.append((node, "\(path)->\(node.val)"))
        }
        // 遇到叶子结点回收结果
        if node.left == nil, node.right == nil { result.append(path) } // 中
    }
    return result
}
```
