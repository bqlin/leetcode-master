...
func sumOfLeftLeaves(_ root: TreeNode?) -> Int {
    ...
    let leftSum = sumOfLeftLeaves(node.left) // 左
    let rightSum = sumOfLeftLeaves(node.right) // 右
    // 中：找到左叶子，记录值
    var value = 0
    if let node = node.left, node.left == nil, node.right == nil {
        value = node.val
    }
...
func sumOfLeftLeaves(_ root: TreeNode?) -> Int {
    ...
    var result = 0
    while !stack.isEmpty {
        let node = stack.removeLast() // 中
        if let node = node.left, node.left == nil, node.right == nil {
            result += node.val
        }
        if let node = node.right { stack.append(node) } // 右
        if let node = node.left { stack.append(node) } // 左
    }
    return result
}
```
