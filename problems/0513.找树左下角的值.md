...
```swift
func traversal(node: TreeNode?, depth: Int, maxDepth: inout Int, value: inout Int!) {
    ...
    if let node = node.left { // 左，需要回溯
        traversal(node: node, depth: depth + 1, maxDepth: &maxDepth, value: &value)
    }
...
func findBottomLeftValue(_ root: TreeNode?) -> Int {
    var maxDepth = -1
    var value: Int!
    traversal(node: root, depth: 0, maxDepth: &maxDepth, value: &value)
    return value
}
```
