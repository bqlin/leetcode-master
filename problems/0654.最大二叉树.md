...
func constructMaximumBinaryTree(_ nums: [Int]) -> TreeNode? {
    ...
    assert(maxValue >= 0 && maxIndex >= 0)

    // 前序构造树
    let root = TreeNode(maxValue)
    if maxIndex > 0 {
        root.left = constructMaximumBinaryTree(Array(nums[0 ... maxIndex - 1]))
    }
...
```swift
func traversal(nums: [Int], leftIndex: Int, rightIdnex: Int) -> TreeNode? {
    guard rightIdnex >= leftIndex else { return nil }

    var maxValue = -1, maxIndex = -1
    for i in leftIndex ... rightIdnex {
        if nums[i] > maxValue {
            maxValue = nums[i]
            maxIndex = i
        }
    ...
    let root = TreeNode(maxValue)
    root.left = traversal(nums: nums, leftIndex: leftIndex, rightIdnex: maxIndex - 1)
    root.right = traversal(nums: nums, leftIndex: maxIndex + 1, rightIdnex: rightIdnex)
    return root
}
func constructMaximumBinaryTree(_ nums: [Int]) -> TreeNode? {
    traversal(nums: nums, leftIndex: 0, rightIdnex: nums.count - 1)
}
```
